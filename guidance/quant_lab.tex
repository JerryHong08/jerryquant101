...
\paragraph{Entry 4 --- Kelly Criterion Is a Leverage Tool, Not an Allocation Tool}
...
\begin{description}[style=nextline, leftmargin=1.8cm]
    ...
    \item[Resolution]
        \begin{enumerate}[nosep]
            ...
            \item \textbf{True Kelly deferred} to a future portfolio-level
                  leverage module that would answer: ``given my
                  composite alpha signal, should I deploy 0.5$\times$
                  or 1.5$\times$ capital today?''
        \end{enumerate}
    ...
    \item[Why It Matters]
        \begin{enumerate}[nosep]
            ...
            \item \textbf{Kelly requires reliable $\mu$ estimates.}
                  With $|\mathrm{IC}| \approx 0.02$, the expected
                  return signal is noise.  Kelly amplifies noise.
                  It becomes useful only when $\mathrm{IC} > 0.05$
                  and preferably with multivariate correlations.
        \end{enumerate}

        \medskip
        \textbf{Interview one-liner:}
        \emph{``Kelly tells you how much to bet, not how to split
        the bet.  Using scalar Kelly for cross-sectional allocation
        ignores correlations and concentrates into low-vol names ---
        exactly the wrong thing for a volatility-driven alpha.''}
\end{description}
...
\begin{description}[style=nextline, leftmargin=1.8cm]
    ...
    \item[Context]
        After fixing the forced-short bug (Entry~5b below) and renaming
        Half-Kelly $\to$ Signal-Weighted (Entry~4), we stepped back to
        examine the position sizing module as a whole.  All four methods
        share the same two-stage architecture:
    ...
    \item[The Forced-Short Problem]
        Setting \texttt{n\_short=10} forces the portfolio to short 10
        stocks even when the signal says they are merely \emph{weaker
        longs}, not actual short candidates.  Our alpha signal is
        directional (higher = better), not symmetric around zero.
        Forcing shorts on a directional signal is a design error.

        \medskip
        \textbf{Fix applied:}  Added \texttt{portfolio\_mode} field to
        \texttt{AlphaConfig} with validation:
        \begin{itemize}[nosep]
            \item \texttt{"long\_only"} (default): \texttt{n\_short=0}.
            \item \texttt{"long\_short"}: requires \texttt{n\_short > 0}.
        \end{itemize}
        Long-only is the correct default for a directional alpha on a
        50-stock universe.  Long-short makes sense only for
        market-neutral strategies with a genuinely symmetric signal.
