...
python src/data_fetcher/splits_fetch.py | tee -a "$LOGFILE"

# ---- 获取所有股票列表 ----
echo "[6/7] 获取最新股票列表..." | tee -a "$LOGFILE"
python src/data_fetcher/all_tickers_fetch.py | tee -a "$LOGFILE"

# ---- fecth versatile tickers ----
echo "===== [7/7] versatile tickers 更新：$(date) =====" | tee -a "$LOGFILE"
python src/data_fetcher/versatile_tickers_fetch.py | tee -a "$LOGFILE"
